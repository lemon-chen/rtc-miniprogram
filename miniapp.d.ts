...
export enum ENCRYPTION_MODE {
  ...
  AES_128_GCM2 = 7,
  /**
   * 8: 256 位 AES 加密，GCM 模式。该加密模式需要设置盐 (`encryptionSalt`)。
   */
  AES_256_GCM2 = 8,
  /**
   * 9: 枚举值边界。
 ...
 * 用户 ID 的类型。
 * @note
 * - 使用字符串作为用户 ID 支持与 Native SDK 2.8 及以上版本或 Web SDK 互通。请确保 Native SDK 使用 User Account 加入频道。详见[如何使用 String 型的用户名](https://doc.shengwang.cn/faq/integration-issues/string-uid)。
 * - 一个频道内的所有用户必须使用同样类型的 uid，即必须都为整数或都为字符串。
 */
export enum UidType {
  /**
   * （默认）32 位无符号整数。建议设置范围：0 到 (2<sup>32</sup>-1)。默认值为 0。
   */
  INT,
  /**
   * 字符串，长度不超过 255 个字符。
